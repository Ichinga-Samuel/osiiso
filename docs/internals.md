...
The library is ~3,700 lines across 14 modules in `src/osiiso/`.  Nothing here
...
| `must_complete` | `False` | every cancellation decision (see §13) |
...
At module level: `TaskState = Literal["pending", "running", "retrying",
"succeeded", "failed", "cancelled"]` — every value `handle.status` can take.
The first three are live states; the final three are exactly `TaskStatus`
from `result.py`, so a finished handle's status always equals its result's
status.  The module also has its own `logger` (`osiiso.handle`), used to
report swallowed done-callback exceptions.
...
- `__repr__` — `TaskHandle('fetch', status='running', id=3f2a9c1b)`-style;
  reads only atomic attributes, so it is safe to log from any thread at any
  time.
...
Flags — the state machine (§13 has the full glossary): `_accepting`,
...
### `join()` and `clear_results()`

`join()` is the minimal wait: start the queue if needed, then
`await _idle.wait()` — no timeout, no summary, no worker teardown.  It is
what you use in an infinite-mode service to wait for a quiet moment without
ending the run.  `clear_results()` empties the accumulated `_results` list
so a long-lived queue does not grow without bound; call it *between* runs —
a `run()` in flight slices `_results` by the index it captured at entry, so
clearing mid-run would skew that window.  (Both exist with identical
semantics on the sync engine, §8.)
...
## 12. `checkpoint.py` — completion tracking keyed by input

The only module that touches a disk the user did not ask about, and the only
one whose absence changes nothing: pass no `checkpoint=` and not a line of it
runs.

**What it is not.** It does not persist tasks.  Nothing about the callable is
written down, so there is no "recover the outstanding queue" story and no
at-most-once guarantee.  It answers exactly one question — *which inputs have I
already finished?* — and the honest failure model is **at-least-once**: the
completion is written *after* the task returns, so a crash in that window
re-runs the input.

**Why not pickle the task?**  A callable pickles as a qualified-name reference,
so closures, lambdas, and locally-defined functions cannot round-trip.  Only
`ProcessQueue` imposes that constraint today (§10); pushing it onto the other
two backends to enable persistence would be a real cost for a feature most runs
do not need.  Keying on the *input* sidesteps it entirely.

### Key derivation

`canonical_key(entry)` is `json.dumps(entry, sort_keys=True,
separators=(",", ":"))`.  Sorted keys make dict ordering irrelevant; JSON makes
the key identical across processes.  `repr()` was rejected outright — it embeds
memory addresses for most objects, so a `repr`-keyed store would silently never
match on resume, which is the worst possible failure for this feature.  A
non-encodable entry raises `TypeError` at *submit* time naming `key=`; failing
loudly beats a checkpoint that quietly does nothing.

Note that tuples encode as JSON lists, so `(1, 2)` and `[1, 2]` share a key —
correct, since `_fan` (§6) treats both as the same positional args.

### Storage

One table, `PRIMARY KEY (namespace, key)`, `WITHOUT ROWID`:

| Column | Purpose |
|---|---|
| `namespace` | isolates one fan-out from another in the same file |
| `key` | the derived input key |
| `value` | encoded return value, `NULL` when `store_values=False` |
| `encoding` | `"json"` or `"none"` — distinguishes a stored `None` from an unstored one |
| `name` | task name, purely so the table reads well in a SQLite CLI |
| `recorded_at` | **`time.time()`, not `perf_counter()`** — this row has to outlive the process that wrote it |

That last row is the whole reason a naive "just persist the queue" design is
harder than it looks: everything else in the library times with
`perf_counter()`, whose epoch is arbitrary per process (§4).

`journal_mode=WAL` + `synchronous=NORMAL` is chosen deliberately: WAL survives
process death, and `NORMAL` skips the per-commit fsync that `FULL` would pay.
The residual risk is OS/power failure, not a crashing Python process — and a
crashing process is precisely the failure this module exists for.  Commit
batching was considered and rejected: it would reopen exactly the window the
module is meant to close.

`sqlite3` is imported **inside `__init__`**, not at module scope, so
`import osiiso` still succeeds on a Python build without it.  `core.py` imports
only `canonical_key` at module level, which is pure stdlib `json`.

### Concurrency

One connection with `check_same_thread=False` behind a `threading.Lock`.
Completions are recorded from whichever thread or loop resolved the task —
worker threads for `ThreadQueue`, the coordinator thread for `ProcessQueue`,
the event loop for `AsyncQueue` — and all three land in the parent process, so
a single lock is sufficient.  `lookup()` collects raw rows under the lock and
**decodes outside it**, since a large value's decode should not block other
workers' recording.

Batched lookups chunk at 500 keys per `IN (...)` to stay well under SQLite's
host-parameter cap.

### Integration with the submission plane

Three pieces in `core.py`, all inert without a checkpoint:

- `_task_key(entry, key_fn, keyable=None)` — user `key_fn` always sees the
  *original* entry; `keyable` is the JSON-encodable stand-in used for
  heterogeneous group entries, which carry a live callable and are keyed as
  `[callable_name, args]`.
- `_enqueue_tracked(plan, opts, cp, namespace)` — one batched `lookup()`, then
  per input either a restored handle or a real `_enqueue` followed by
  `cp._watch(handle, ns, key)`.
- `_restored_handle(fn, opts, hit)` — builds a handle, calls `_mark_finished`
  with a `succeeded` result, and hands it back.  `cancel_fn` is
  `_never_cancels`; it is never invoked, because `cancel()` short-circuits on
  `done()` (§5).

`_watch` attaches an `add_done_callback` that records **only** on
`status == "succeeded"`.  Because `add_done_callback` fires immediately for an
already-finished handle (§5), there is no race between `_enqueue` returning and
the callback being attached — which matters for the sync queues, whose workers
may already be running.

### Two invariants worth remembering

1. **A restored task never reaches `_record`.**  It is never enqueued, so
   `_outstanding` is untouched and it is absent from the queue's `_results` —
   and therefore from the `RunSummary`.  The summary describes work *performed
   this run*.  Group values come from handles (§11), so `group.values()` still
   aligns 1:1 with the input.  Code that reads `summary.values` instead will
   silently lose every restored item; this is the one sharp edge of the feature
   and is called out in the guide.
2. **Only successes are recorded.**  Failures, cancellations, and tasks that
   never ran leave no row, so the next pass retries them.  This is what lets
   `retries=` and `checkpoint=` compose: retries handle transient failure
   *within* a run, the checkpoint handles it *across* runs.

### Failure handling

Both encode/decode failures fail **safe**, toward re-running:

- an unencodable return value logs a warning and records nothing, so the input
  runs again (rather than recording a completion whose value is lost);
- an undecodable row is omitted from `lookup()` and logged, so the input runs
  again (rather than handing back garbage).

`namespace` defaults to the callable's name for the mapped forms.  For
`group([(fn, *args), ...])` it is **required**, because the fallback would be
the generated `group-{n}` id, which changes every run and would never match —
`ValueError` at submit time rather than a checkpoint that appears to work and
never resumes.
...
## 14. Tests, tooling, and how to work on this
...
- **src layout**: the package lives in `src/osiiso/`; setuptools is pointed
  at it via `package-dir "" = "src"` in `pyproject.toml`.  You must install
  the package (`uv sync` does an editable install) — you cannot import it
  from a bare checkout.
- **Zero runtime dependencies**, `requires-python >= 3.13`.  The code uses
  PEP 695 generics (`def run[T](...)` in `loop.py`), `X | None` unions, and
  `slots=True` dataclasses throughout — do not "fix" these for older
  Pythons.
- **`py.typed`** ships in the package, so the inline annotations are the
  type stubs; keep signatures accurate.
- **Optional extras**: `osiiso[uvloop]` (picked up automatically by
  `osiiso.run`), plus `docs` and `dev` extras for contributors.
- **Lint/format**: ruff with `line-length = 135` and rules `B, E, F, I, UP`;
  the formatter runs with `skip-magic-trailing-comma`.  Run both before
  committing (commands below).
- **Docs**: mkdocs-material; this page is registered in `mkdocs.yml` under
  **Internals → Codebase Walkthrough**.
